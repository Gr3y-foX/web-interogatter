# 🚀 Установка на Raspberry Pi

Полная инструкция по установке и запуску Web Server Interceptor на Raspberry Pi.

## 📋 Требования

- **Raspberry Pi 4** (рекомендуется 4GB RAM или больше)
- **Raspberry Pi OS (64-bit)** - Bullseye или новее
- **Интернет-соединение**
- **SSH доступ** (для удаленной установки)

## 🎯 Быстрая установка (рекомендуется)

### Способ 1: Автоматическая установка из git

```bash
# Если у вас есть git URL репозитория
curl -fsSL https://raw.githubusercontent.com/YOUR_USERNAME/web-server-intercepter/master/raspberry-production/install_and_run.sh | bash -s -- YOUR_GIT_URL

# Или клонируйте и запустите
git clone YOUR_GIT_URL ~/web-server-intercepter
cd ~/web-server-intercepter/raspberry-production
chmod +x install_and_run.sh
./install_and_run.sh
```

### Способ 2: Если проект уже клонирован

```bash
cd ~/web-server-intercepter/raspberry-production
./install_and_run.sh
```

Скрипт `install_and_run.sh` автоматически:
1. ✅ Обновит проект из git (или клонирует, если нет)
2. ✅ Установит системные зависимости
3. ✅ Установит Docker и Docker Compose
4. ✅ Установит Python зависимости
5. ✅ Настроит проект
6. ✅ Запустит сервер

## 📖 Детальная установка

### Шаг 1: Подготовка Raspberry Pi

```bash
# Обновление системы
sudo apt update && sudo apt upgrade -y

# Установка базовых пакетов
sudo apt install -y git curl wget
```

### Шаг 2: Клонирование проекта

```bash
# Переход в домашнюю директорию
cd ~

# Клонирование репозитория
git clone YOUR_GIT_URL web-server-intercepter
cd web-server-intercepter/raspberry-production
```

### Шаг 3: Запуск установки

```bash
# Сделать скрипт исполняемым
chmod +x install_and_run.sh

# Запуск установки
./install_and_run.sh
```

### Шаг 4: Настройка безопасности (опционально, но рекомендуется)

```bash
# Усиленная настройка безопасности
sudo ./setup_security.sh

# Настройка firewall
sudo ./setup_production_firewall.sh 5000
```

## 🔄 Обновление проекта

### Автоматическое обновление

```bash
cd ~/web-server-intercepter/raspberry-production
./install_and_run.sh
```

Скрипт автоматически:
- Сохранит локальные изменения в stash (если есть)
- Обновит проект из git
- Перезапустит сервер

### Ручное обновление

```bash
cd ~/web-server-intercepter

# Сохранение изменений (если есть)
git stash

# Обновление из git
git pull origin master

# Перезапуск сервера
cd raspberry-production
./raspberry-run.sh restart
```

## 🎮 Управление сервером

### Запуск и остановка

```bash
cd ~/web-server-intercepter/raspberry-production

# Запуск
./raspberry-run.sh start

# Остановка
./raspberry-run.sh stop

# Перезапуск
./raspberry-run.sh restart

# Статус
./raspberry-run.sh status
```

### Просмотр логов

```bash
# Все логи
./raspberry-run.sh logs

# Логи конкретного сервиса
./raspberry-run.sh logs interceptor
```

### Мониторинг

```bash
# Мониторинг безопасности
./monitor_security.sh

# Мониторинг ресурсов
./raspberry-run.sh monitor
```

## 🌐 Доступ к серверу

После запуска сервер будет доступен:

- **Локально**: `http://localhost:5000`
- **В сети**: `http://[IP-адрес]:5000`
- **Tor Hidden Service**: `http://[onion-адрес]` (получить: `./raspberry-run.sh onion`)

Узнать IP адрес:
```bash
hostname -I
```

## 🔒 Настройка безопасности

### Базовая безопасность

```bash
# Изменение пароля
passwd

# Настройка SSH ключей (рекомендуется)
ssh-keygen -t ed25519
```

### Усиленная защита

```bash
# Полная настройка безопасности
sudo ./setup_security.sh

# Настройка firewall
sudo ./setup_production_firewall.sh 5000
```

Это настроит:
- fail2ban (защита от брутфорса)
- Усиленный firewall
- Автоматические обновления
- Мониторинг безопасности
- Автоматическое резервное копирование

## 🐛 Решение проблем

### Проблема: Git не может клонировать

```bash
# Проверка интернет-соединения
ping -c 3 8.8.8.8

# Проверка git
git --version

# Установка git
sudo apt install -y git
```

### Проблема: Docker не запускается

```bash
# Проверка Docker
docker --version

# Запуск Docker
sudo systemctl start docker
sudo systemctl enable docker

# Проверка прав
groups $USER
# Должна быть группа 'docker'
# Если нет: sudo usermod -aG docker $USER
# И перезайти в систему
```

### Проблема: Порт занят

```bash
# Найти процесс
sudo lsof -i :5000

# Остановить процесс
sudo kill -9 [PID]
```

### Проблема: Недостаточно памяти

```bash
# Проверка памяти
free -h

# Увеличение swap
sudo dphys-swapfile swapoff
sudo nano /etc/dphys-swapfile
# CONF_SWAPSIZE=2048
sudo dphys-swapfile setup
sudo dphys-swapfile swapon
```

## 📝 Дополнительная информация

- **Основная документация**: [README.md](README.md)
- **Настройка безопасности**: [setup_security.sh](setup_security.sh)
- **Мониторинг**: [monitor_security.sh](monitor_security.sh)

## ⚠️ Важные замечания

1. ⚠️ **Убедитесь, что SSH доступен** перед настройкой firewall
2. ⚠️ **Используйте Tor Hidden Service** для публичного доступа
3. ⚠️ **Регулярно обновляйте систему**: `sudo apt update && sudo apt upgrade`
4. ⚠️ **Делайте резервные копии**: автоматические копии в `/var/backups/web-interceptor/`

---

**🎓 Образовательный проект по кибербезопасности**  
*Используйте ответственно и в соответствии с законами*

